...
# noinspection PyUnusedLocal,PyShadowingNames,PyUnboundLocalVariable,PyTypeChecker
class imagelanedetectionnode:
    ...
    def callback(self, data, miss):
        global left_fit, right_fit, right, left
        try:
        ...
        transform_src = np.float32([[100, 120], [540, 120], [640, 400], [0, 400]])  # Festlegung ROI
        ...
        if len(rightlist) < 10:
            ...
            if abs(rightlist[9] - rightlist[8]) > 200:  # Abweichung der beiden neusten Spurpunkte > 200px, dann setze Korrekturwert
                # print rightlist
                miss[0] = 1
        ...
        if len(leftlist) < 10:
            ...
            leftlist.append(left)
            leftlist.pop(0)
            if abs(leftlist[9] - leftlist[8]) > 200:  # Abweichung der beiden neusten Spurpunkte > 200px, dann setze Korrekturwert
                miss[1] = 1
                # print leftlist[9]-leftlist[8]

        if len(leftlist) == 10 and (rightlist[9] - leftlist[9]) < 200:  # Wenn aktuelle Spurbreite < 200px
            if rightlist[9] < 320 and leftlist[9] < 320:  # Wenn Fahrspuren in linker Bildhaelfte
                rightlist[9] = 640  # Korrekturwert rechts max annaehern

            elif rightlist[9] > 320 and leftlist[9] > 320:  # Wenn Fahrspuren in rechter Bildhaelfte
                leftlist[9] = 0  # Korrekturwert links max annaehern

        # if len(rightlist)==10 and abs(rightlist[9]-rightlist[8])>4:
        #    rightlist[9] = rightlist[8]+((-1 if((rightlist[9]-rightlist[8])<0) else 1)*4)
        # if len(leftlist)==10 and abs(leftlist[9]-leftlist[8])>4:
        ...
        # TODO: Bereich zwischen -8 ... +8 anpassen
        ...
        # Polynominterpolation fuer rechte und linke Spur
        ...
        if leftx.size != 0 and lefty.size != 0 and rightx.size != 0 and righty.size != 0:
            ...
            # TODO: Wirft Error, wenn keine Spur bei Programmstart. Abfangen in Schleife?
